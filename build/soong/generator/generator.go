...
func init() {
	android.RegisterModuleType("fulia_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = fuliaExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = fuliaExpandVariables(ctx, dep_file)
	...
	cmd := fuliaExpandVariables(ctx, String(g.properties.Cmd))
